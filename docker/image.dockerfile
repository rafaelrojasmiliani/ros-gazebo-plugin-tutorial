...
# Install packages
RUN DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends -o Dpkg::Options::="--force-confnew" \
                    ros-melodic-desktop-full iputils-ping net-tools netcat screen build-essential ros-melodic-ros-control \
                    ros-melodic-gazebo-ros-control ros-melodic-hardware-interface libgazebo9-dev python-catkin-tools
# user handling
ARG myuser
ARG myuid
ARG mygroup
